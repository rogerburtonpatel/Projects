 ...
 **   April 2021: Roger A. Burtonpatel
 **       Added report_path and get_vertex_neighbors functions with seperate 
...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist &artist) const
{
    ...
    enforce_valid_vertex(artist); // checks artist validity in graph 
    vector<Edge> all_neighbors = graph.at(artist.get_name())->neighbors;

    for (size_t i = 0; i < all_neighbors.size(); ++i) {
        if (get_edge(artist, all_neighbors[i].neighbor) != "")
            result.push_back(all_neighbors[i].neighbor); // populates vector
    }
    return result;
}
                                            ...
                                            const Artist &dest) const
{
    ...
    enforce_valid_vertex(source); // checks validity 
    enforce_valid_vertex(dest);
    bool found_source = false; 

    Artist a = dest;
    // Second condition for if no path is found 
    while (found_source == false and a.get_name() != "") {
        path.push(a);
        ...
        if (a == source) {
            path.push(a);
            found_source = true; // if the bottom is successfuly reached
        }
    } 
    // Ensures return of empty stack if no valid path exists
    if (not found_source) {
