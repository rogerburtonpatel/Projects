...
void gerp::read_file(const std::string &filepath, size_t &lineindex, size_t &fileindex)
{    
    size_t linenumber = 1;
    std::string word; 
    std::string fullline;
...
void gerp::get(const std::string &Key, std::ofstream &output, const bool &insensitive)
{
    ...
    for (const auto & wp : found_wps) {
        print(wp, output);
    }
}
// TODO add references
void gerp::print(const wordPod &wp, std::ofstream &output)
{
